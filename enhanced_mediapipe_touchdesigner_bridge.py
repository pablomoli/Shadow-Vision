...
class EnhancedTouchDesignerBridge:
    ...
    def __init__(self, td_ip="127.0.0.1", td_port=7000,
        ...
        # Mapping required by user: dog, cat, deer, swan, rabbit, bird, snail → 7010-7016
        self.label_port_map = {
            'dog': 7010,
            'cat': 7011,
            'deer': 7012,
                ...
                if animal_key in self.label_clients:
                    ...
                    try:
                        ...
                        if self.verbose_label_logging:
                            logger.info(f"Label '{animal_key}' → port {self.label_port_map[animal_key]} /shadow_puppet/selected_label")
                        else:
